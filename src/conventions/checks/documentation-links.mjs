...
function repositoryPath(path) { return posix.normalize(path.replaceAll('\\', '/')).replace(/^\.\//, '').replace(/^\//, ''); }
...
export function hasAnyFileLink(texts, target) {
  const normalizedTarget = repositoryPath(target);
  for (const [source, text] of texts) {
    const directory = posix.dirname(repositoryPath(source));
