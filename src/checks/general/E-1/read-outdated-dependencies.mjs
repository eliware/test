...
export function readOutdatedDependencies(root, spawnProcess = spawn) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (oversized)
        return reject(new Error(`npm outdated output exceeded ${maxStdoutLength} characters.`));
      if (code !== 0 && !stdout.trim())
        return reject(new Error(stderr || `npm outdated exited with ${code}.`));
      try {
        resolve(JSON.parse(stdout || "{}"));
      } catch {
        reject(new Error("npm outdated returned invalid JSON."));
      }
    });
  });
}
