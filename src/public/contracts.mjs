...
export function assertToolkitOptions(options, operation = 'toolkit') {
  ...
  for (const name of ['runTest', 'runLintCommand', 'accessPath', 'removePath', 'readFilePath', 'findIstanbulIgnores', 'findMonolith', 'inspectWorkspace']) {
    if (name in options && options[name] !== undefined && typeof options[name] !== 'function') throw new TypeError(`${operation} option ${name} must be a function`);
  }
  for (const name of ['runInBand', 'ignoreCoverage', 'ignoreMonolithLimits', 'enforceMonolithLimits', 'debugTiming']) {
    if (name in options && options[name] !== undefined && typeof options[name] !== 'boolean') throw new TypeError(`${operation} option ${name} must be boolean`);
  }
  return options;
}
