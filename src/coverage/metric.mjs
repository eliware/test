export function metricHasGap(value) {
  ...
  if (annotated) {
    ...
    if (annotated[2].length > 256 || annotated[3].length > 256) return true;
    const displayed = percentageHundredths(annotated[1]); const covered = BigInt(annotated[2]); const total = BigInt(annotated[3]); if (total <= 0n || covered > total) return true; return covered !== total || displayed === null || displayed !== (covered * 10000n + total / 2n) / total;
  }
...
function percentageHundredths(value) { const [whole, fraction = ''] = value.split('.'); if (whole.length > 3 || BigInt(whole) > 100n || (whole === '100' && /[1-9]/.test(fraction)) || fraction.length > 1024) return null; let result = BigInt(whole) * 100n + BigInt(fraction.slice(0, 2).padEnd(2, '0')); if (fraction.length > 2 && fraction[2] >= '5') result += 1n; return result; }
