...
test("requires npm ci before npm test", () => {
  expect(validateWorkflowSequence("ci.yml", [{ command: "npm test" }])).toBe("ci.yml must validate with exactly one npm ci followed immediately by npm test.");
  expect(validateWorkflowSequence("ci.yml", [{ command: "npm test" }, { command: "npm ci" }])).toContain("followed immediately");
  expect(validateWorkflowSequence("ci.yml", [{ command: "npm ci" }, { command: "npm test" }])).toBeNull();
  expect(validateWorkflowSequence("ci.yml", [{ command: "npm ci" }, { command: "npm test" }], null)).toBeNull();
});
...
test("rejects intervening, duplicate, mutating, or skippable workflow steps", () => {
  ...
  ], [install, { run: "npm ci" }, test])).toContain("exactly one");
  expect(validateWorkflowSequence("ci.yml", [
    { command: install.run, index: 0, step: install },
    { command: test.run, index: 1, step: { ...test, "continue-on-error": true } },
  ], [install, { ...test, "continue-on-error": true }])).toContain("no intervening steps");
  const skippedTest = { ...test, "continue-on-error": true };
  expect(validateWorkflowSequence("ci.yml", [
    { command: install.run, index: 0, step: install },
    { command: skippedTest.run, index: 1, step: skippedTest },
  ], [install, skippedTest])).toContain("ignore failure");
});
...
test("rejects conditionally skipped validation jobs and install steps", () => {
  ...
  expect(validateWorkflowSequence("ci.yml", commands, [install, testStep], { if: "false" }))
  ...
  expect(validateWorkflowSequence("ci.yml", [
    ...
    { command: testStep.run, index: 1, step: testStep },
  ], [conditionalInstall, testStep])).toContain("conditionally skip");
});
