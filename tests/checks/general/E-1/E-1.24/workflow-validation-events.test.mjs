...
test("evaluates supported push and runner shapes", () => {
  ...
  expect(workflowHasValidationEvents({ on: { ...events, push: { branches: ["main"], "branches-ignore": ["main"] } }, jobs })).toBe(false);
  expect(workflowHasValidationEvents({ on: { ...events, push: { branches: ["main", "!main"] } }, jobs })).toBe(false);
  expect(workflowHasValidationEvents({ on: { ...events, push: { branches: ["main"], "branches-ignore": ["release/*", null] } }, jobs })).toBe(true);
  expect(workflowHasValidationEvents({ on: { ...events, push: { branches: ["*", "!main", "main"] } }, jobs })).toBe(true);
  expect(workflowHasValidationEvents({ on: { ...events, push: { branches: ["*", null, "!release/*"] } }, jobs })).toBe(true);
  expect(workflowHasValidationEvents({ on: { ...events, push: { branches: [], "branches-ignore": [] } }, jobs })).toBe(true);
  expect(workflowHasValidationEvents({ on: { ...events, push: { branches: ["*"], "branches-ignore": ["m*"] } }, jobs })).toBe(false);
