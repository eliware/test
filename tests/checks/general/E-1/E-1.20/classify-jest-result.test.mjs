...
test("classifies timeout, failure, and success results", () => {
  expect(classifyJestResult("E-1.130.13", { timedOut: true }, "timed out").status).toBe("fail");
  expect(classifyJestResult("E-1.130.13", { timedOut: true }).message)
    .toBe("Jest timed out after 15 seconds without progress.");
  expect(classifyJestResult("E-1.130.13", { code: 1, stdout: "out", stderr: "err" }).message)
    .toBe("Jest failed: out\nerr");
  expect(classifyJestResult("E-1.130.13", { code: 1, stdout: "", stderr: "" }).message)
    .toBe("Jest failed without diagnostics.");
  expect(classifyJestResult("E-1.130.13", { code: 1 }).message)
    .toBe("Jest failed without diagnostics.");
  expect(classifyJestResult("E-1.130.13", { code: 0 }).status).toBe("pass");
});

test("removes routine passing and coverage output from failure diagnostics", () => {
  expect(classifyJestResult("E-1.130.13", {
    code: 1,
