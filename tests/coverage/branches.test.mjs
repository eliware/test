...
test('handles missing branch locations', () => expect(uncoveredBranches({ a: { type: 'if', locations: [] } }, 'a', [0])).toEqual([{ type: 'if' }]));
test('handles absent location arrays', () => expect(uncoveredBranches({ a: { type: 'if' } }, 'a', [0])).toEqual([{ type: 'if' }]));
test('handles null location arrays', () => expect(uncoveredBranches({ a: { type: 'if', locations: null } }, 'a', [0])).toEqual([{ type: 'if' }]));
test('uses a generic type when branch type is absent', () => expect(uncoveredBranches({ a: { locations: [] } }, 'a', [0])).toEqual([{ type: 'branch' }]));
