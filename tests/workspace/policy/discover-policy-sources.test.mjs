...
test('discovers supported files while skipping generated directories', async () => {
  const entries = {
    ...
    'C:/repo/src': [{ name: 'module.mjs', isDirectory: () => false, isFile: () => true }, { name: 'data.json', isDirectory: () => false, isFile: () => true }]
  };
  await expect(discoverPolicySources('C:/repo', async (path) => entries[path.replaceAll('\\', '/')] ?? [])).resolves.toEqual([{ root: 'C:\\repo', path: 'C:\\repo\\src\\module.mjs' }]);
});
