...
test('validates options and delegated exit codes', async () => {
  ...
  await expect(runLint({ cwd: 'C:/repo', runLintCommand: async () => 1.5 })).rejects.toThrow('integer exit code');
  await expect(runLint({ cwd: 'C:/repo', runLintCommand: async () => 7 })).resolves.toBe(7);
});
